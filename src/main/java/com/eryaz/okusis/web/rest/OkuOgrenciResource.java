...
public class OkuOgrenciResource {
    ...
    public ResponseEntity<OkuOgrenci> createOkuOgrenci(@RequestBody OkuOgrenci okuOgrenci) throws URISyntaxException {
        ...
        if (okuOgrenci.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new okuOgrenci cannot already have an ID")).body(null);
        }
    ...
    @Timed
    public List<OkuOgrenci> getAllOkuOgrencis() {
        log.debug("REST request to get all OkuOgrencis");
        return okuOgrenciService.findAll();
        }
