...
public class OkuSoforResource {
    ...
    public ResponseEntity<OkuSofor> createOkuSofor(@RequestBody OkuSofor okuSofor) throws URISyntaxException {
        ...
        if (okuSofor.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new okuSofor cannot already have an ID")).body(null);
        }
    ...
    @Timed
    public List<OkuSofor> getAllOkuSofors() {
        log.debug("REST request to get all OkuSofors");
        return okuSoforService.findAll();
        }
